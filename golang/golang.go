...
func SetupGolang(version string) (string, error) {
	...
	if !isGoInstalled(goDir) {
		...
		if err := os.Mkdir(filepath.Join(home, ".bake"), 0755); err != nil {
			if !os.IsExist(err) {
				return "", err
			}
		}
...
func golangDir(home, version, goos, goarch string) (string, error) {
	return filepath.Join(home, ".bake", fmt.Sprintf("go%s.%s.%s", version, goos, goarch)), nil
}
...
func isGoInstalled(goDir string) bool {
	...
	_, err := os.Stat(filepath.Join(goDir, "bin", "go"+ext))
	if err != nil {
		return false
	}
